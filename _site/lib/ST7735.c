...
* @fn   void SPI_ControllerTx_16bit(uint16_t data)
* @brief  Send 16-bit data to peripheral
* @note   Used for color information(Without Begin and End) and Send to set position
*****************************************************************************/
void SPI_ControllerTx_16bit_NoBE(uint8_t x0, uint8_t y0,uint16_t data){
    uint8_t ST7735_cmds[]  =
 {
  ST7735_CASET, 4, 0x00, x0, 0x00, x0, 0,  // Column
  ST7735_RASET, 4, 0x00, y0, 0x00, y0, 0,  // Page
...
void SPI_ControllerTx_16bit_S(uint16_t data, int32_t num_pixels)
{
	uint8_t temp = data >> 8;
	clear(LCD_PORT, LCD_TFT_CS);	//CS pulled low to start communication
